...
class Para:
    """ Class of parameters """
    def __init__(self, log_format, rex, rex_s_token, log_name, tmplt_lib,
                 indir='./', outdir='./', pstdir='./', max_child=120,
                 sim_t_m=1, inc_updt=1, over_wr_lib=False, prt_tree=0,
                 nopgbar=0):
        """
        Attributes
        ----------
        log_format  : used to load the needed colomns of raw logs
        log_name    : the name of the input raw file
...
class Drain:
    ...
    def output_result(self, log_clust_lst):
        ...
        df_event.to_csv(os.path.join(self.para.save_path, self.para.log_name + '_templates.csv'),
        ...
        if self.para.over_wr_lib and self.para.inc_updt:
            if os.path.exists(os.path.join(self.para.pstdir, self.para.tmplt_lib)):
                shutil.copy(os.path.join(self.para.pstdir, self.para.tmplt_lib), \
                            os.path.join(self.para.pstdir, self.para.tmplt_lib+'.old'))
            df_event.to_csv(os.path.join(self.para.pstdir, self.para.tmplt_lib), \
        ...
        self.df_log.to_csv(
            os.path.join(self.para.save_path, self.para.log_name + '_structured.csv'),
            index=False
        )
    ...
    def load_data(self):
        ...
        self.df_log = self.log_to_dataframe(os.path.join(self.para.path, self.para.log_name),
                                            regex, headers)
    ...
    def load_template_lib(self):
        ...
        """
        if self.para.inc_updt and \
            os.path.exists(os.path.join(self.para.pstdir, self.para.tmplt_lib)):
            self.df_tmp = pd.read_csv(os.path.join(self.para.pstdir, self.para.tmplt_lib),
    ...
    def main_process(self):
        ...
        The main entry
        """
        print('Parsing file: ' + os.path.join(self.para.path, self.para.log_name))
        ...
        if self.para.prt_tree:
            ...
            with open(self.para.save_path + 'tree.txt', 'w') as drain_tree:
